...
email_widget:set_text("hure")

email_icon = wibox.widget.imagebox()
email_icon:set_image(path_to_icons .. "/mail-mark-new.png")

watch(
    "python /home/jan/.config/awesome/email-widget/count_unread_emails.py", 20,
    function(_, stdout)
...
local function show_emails()
    awful.spawn.easy_async([[bash -c 'python /home/jan/.config/awesome/email-widget/read_unread_emails.py']],
        function(stdout)
            naughty.notify{
                text = stdout,
                title = "Unread Emails",
